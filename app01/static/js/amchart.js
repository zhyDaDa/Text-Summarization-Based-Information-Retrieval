$(function () {
  var illness = new URLSearchParams(window.location.search).get('illness');
  console.log(illness);
  if (illness) {
    performActionBasedOnIllness(illness);
  }
  var data = {};
  var myChart = null;
  ...
  window.searchNeo4j = (illness) => {
    var url = '/neo4j/?illness=' + encodeURIComponent(illness);
    window.location.href = url;  // 跳转并传递参数
  }
  function performActionBasedOnIllness(illness) {
    console.log("1");
    $.ajax({
      url: '/neo4j/', // Django 视图函数的 URL 路由
      type: 'POST',
      ...
      success: function (response) {
        ...
        updateGraph(data);
        console.log("shit");
        //window.location.href = '/neo4j/';
      },
    });
  }
  // window.searchNeo4j = (text) => {

  //   console.log("3");
  //   var rowContent = text
  //   console.log("2");
  //   var url = '/neo4j/?rowContent=' + encodeURIComponent(rowContent);
  //   window.location.href = url; // 跳转并传递参数
  //   $.ajax({
  //     url: '/neo4j/', // Django 视图函数的 URL 路由
  //     type: 'POST',
